...
class SwinTransformer(nn.Module):
    ...
    def load_weights(self, pretrained_dict=None, pretrained_layers=[], verbose=True, rename=False):
        model_dict = self.state_dict()
        
        if rename:
            pretrained_dict = {
                k.replace("encoder.", ""): v for k, v in pretrained_dict.items() if "encoder." in k
            }
        ...
        missing_keys, unexpected_keys = self.load_state_dict(need_init_state_dict, strict=False)
...
def get_swin_backbone(cfg):
    ...
    if cfg['MODEL']['BACKBONE']['LOAD_PRETRAINED'] is True:
        ...
        swin.load_weights(ckpt, swin_cfg.get('PRETRAINED_LAYERS', ['*']), cfg['VERBOSE'], rename=rename)
